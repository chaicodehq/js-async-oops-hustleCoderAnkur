...
export class Batsman extends Player {
  ...
  getStrikeRate() {
    ...
    const strikeRateTotal = this.innings.reduce((sum, obj) => {
      return sum + obj.strikeRate
    }, 0)

    return strikeRateTotal / this.innings.length
  }

  getProfile() {
    const totalRuns = this.innings.reduce((sum, obj) => {
      return sum + obj.runs
    }, 0)
...
export class Bowler extends Player {
  ...
  getEconomy() {
    ...
    const totalEconomy = this.spells.reduce((sum, obj) => {
      return sum + obj.economy
    }, 0)
  ...
  getProfile() {
    ...
    const totalWickets = this.spells.reduce((sum, obj) => {
      return sum + obj.wickets
    }, 0)
